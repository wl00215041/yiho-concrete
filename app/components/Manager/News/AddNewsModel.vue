    ...
    <div class="flex flex-col gap-4">
      <form @submit.prevent="add">
        <div class="flex flex-col gap-4 mb-6">
          ...
          <input id="name" class="py-3 px-6" type="text" v-model="payload.title" placeholder="請輸入標題" required />
        </div>
        <div class="flex flex-col gap-4 mb-6">
          <label class="text-[#1C2434] font-medium mb-[10px]" for="link">連結<Required></Required></label>
          <input id="link" class="py-3 px-6" type="text" v-model="payload.link" placeholder="請輸入連結" required />
        </div>
